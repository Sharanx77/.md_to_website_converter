...
import { 
  ...
  Loader2, Cpu, Zap, Layers, Code2, Globe
} from 'lucide-react';
...
export default function EchoCompiler() {
  ...
  const generateHTMLWrapper = () => {
    ...
    // We inject Tailwind and Typography via CDN for the exported standalone file
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body class="${THEMES[currentTheme].wrapper} min-h-screen p-8 md:p-16">
    ...
    <footer style="margin-top: 5rem; text-align: center; opacity: 0.5; font-family: sans-serif; font-size: 0.75rem;">
      Built with ECHO Compiler by ${myName} & ${aiPartner}
    </footer>
</body>
</html>`;
  };
  ...
  const deployToGithub = async () => {
    ...
    try {
      ...
      // 1. Try to create repo (ignore if exists)
      try {
        await octokit.rest.repos.createForAuthenticatedUser({ 
          name: repoName, 
          auto_init: true,
          description: "My Portfolio generated by ECHO Compiler"
        });
        await new Promise(r => setTimeout(r, 2000)); // Wait for GitHub to initialize
      } catch (e) {}
      ...
      if (window.confirm(`Successfully Pushed!\n\nYour site will be live at:\n${pagesUrl}\n\nNote: GitHub Pages can take up to 60 seconds to refresh. Open link now?`)) {
    ...
    <div className={`flex flex-col h-screen transition-all duration-500 ${THEMES[currentTheme].wrapper}`}>
      ...
      <main className="flex-1 flex flex-col lg:flex-row overflow-hidden p-4 gap-4">
        ...
        <section className={`flex-1 flex flex-col rounded-3xl border transition-all duration-300 shadow-xl overflow-hidden ${THEMES[currentTheme].card}`}>
          <div className="px-5 py-2 border-b border-slate-500/10 bg-slate-500/5 flex justify-between">
            ...
            <span className="text-[8px] font-mono opacity-30">v2.1</span>
          </div>
          <textarea 
          ...
          <div className="flex-1 overflow-y-auto p-6 md:p-12 custom-scrollbar">
            <div id="preview-content" className={`prose ${THEMES[currentTheme].prose} max-w-none`}>
              <ReactMarkdown 
                ...
                components={{
                  // FIXED: Removed 'inline' and added check for 'language-' match for TS compatibility
      ...
      <footer className="px-8 py-4 border-t border-slate-500/10 flex flex-col md:flex-row items-center justify-between text-[10px] font-bold uppercase tracking-[0.2em] gap-4">
        <div className="flex items-center gap-4 opacity-30">
          <span className="flex items-center gap-1.5"><Cpu size={12}/> STABLE_BUILD</span>
        </div>
